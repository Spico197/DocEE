...
def reveal_instances_from_guessing(pred_types: set, mspans: List[dict], field2class: DefaultDict[str, set], guess_strategy: Optional[str] = '&'):
    event_list = []
    if len(pred_types) <= 0:
        # needs type guessing
        guessed_types = set()
            ...
            if 'Trigger' not in pair['mtype']:
                ...
                if len(guessed_types) <= 0:
                    ...
                    if guess_strategy == '&':
                        ...
                        raise ValueError(f"guess_strategy: {guess_strategy} not supported")
...
if __name__ == "__main__":
    ...
    if add_IPO_stage:
        ...
        doc_id2IPO_stage_result = create_doc_id2IPO_index(load_line_json(IPO_stage_result_filepath))
            ...
            if len(d['event_list']) <= 0:
                if add_guessing_instance:
                    ...
                    event_list = reveal_instances_from_guessing(
                        pred_types, d['comments']['mspans'], field2class, guess_strategy=guess_strategy)
                else:
                    pass
            else:
                for ins in d['event_list']:
                    ...
                    if add_additional_arg:
                        ...
                        for pair in d['comments']['mspans']:
                            ...
                            if (pair['mtype'] not in exist_roles
                                    and tmp_arg not in arguments
                                    and pair['mtype'] in event_type2event_class[ins['event_type']].FIELDS):
                                exist_roles.add(pair['mtype'])
                                arguments.append(tmp_arg)
                    ins['arguments'] = arguments
                    if add_IPO_stage and ins['event_type'] == "公司上市" and all(x['role'] != '环节' for x in ins['arguments']) and d['id'] in doc_id2IPO_stage_result:
                        ins['arguments'].append({"role": "环节", "argument": doc_id2IPO_stage_result[d['id']]})
