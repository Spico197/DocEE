...
def render_sentences(mspans, sentences, join_char: Optional[str] = "", ent_class: Optional[str] = None, ent_type_class: Optional[str] = None):
    rendered_sentences = []
...
def match_arg(sentences: List[str], doc_token_ids: np.ndarray, arg: Tuple[int], offset: Optional[int] = 0):
    ...
    if row is not None and col is not None:
        return "".join(sentences[row][col - offset: col + arg_len - offset]), [row, col - offset, col + arg_len - offset]
    else:
        return None, None
...
def prepare_doc_batch_dict(doc_fea_list):
    doc_batch_keys = ['ex_idx', 'doc_type', 'doc_token_ids', 'doc_token_masks', 'doc_token_labels', 'valid_sent_num']
    doc_batch_dict = {}
...
def measure_dee_prediction(
    ...
    if len(features) > 0:
        if all(isinstance(feature, DEEArgRelFeature) for feature in features):
            is_cg = True
        elif all(isinstance(feature, DEEMultiStepTriggeringFeature) for feature in features):
            is_cg = True
        ...
        for term in filtered_event_decode_results:
            ...
            if is_cg:
                ...
                if event_independent:
                    gold_adj_mats.append([mat.reveal_adj_mat() for mat in doc_fea.span_rel_mats])
                else:
                ...
                for combinations in event_idx2combinations:
                    combinations = [tuple(sorted([doc_span_info.span_token_tup_list[arg] for arg in comb])) for comb in combinations]
                ...
                doc_fea.event_arg_idxs_objs_list = remove_event_obj_roles(doc_fea.event_arg_idxs_objs_list, event_type_fields_pairs)
...
def aggregate_task_eval_info(eval_dir_path, max_epoch=100,
                             target_file_pre='dee_eval', target_file_suffix='.json',
...
def print_total_eval_info(doc_type2data_span_type2model_str2epoch_res_list,
                          template,
                          doc_type='overall',
                          metric_type='micro',
                          span_type='pred_span',
    ...
    dev_model_str2epoch_res_list = doc_type2data_span_type2model_str2epoch_res_list[doc_type][(dev_type, span_type)]
    test_model_str2epoch_res_list = doc_type2data_span_type2model_str2epoch_res_list[doc_type][(test_type, span_type)]

    has_header = False
    mstr_bepoch_list = []
    logger.info(f"{'-'*15} doc_type: {doc_type}, part: {part}, span_type: {span_type} {'-'*15}")
    logger.info('=' * 15, 'Final Performance (%) (avg_type={})'.format(metric_type), '=' * 15)
...
def print_single_vs_multi_performance(
    mstr_bepoch_list, base_dir, features, template,
    event_independent,
    ...
    for model_str, _ in mstr_bepoch_list:
        ...
        etype_sf1_mf1_list = []
        for event_idx, (se_res, me_res) in enumerate(zip(single_eval_res[part]['Events'], multi_eval_res[part]['Events'])):
    ...
    logger.info('=' * 15, 'Single vs. Multi (%) (avg_type={})'.format(metric_type), '=' * 15)
    align_temp = '{:20}'
    head_str = align_temp.format('ModelType')
    head_temp = ' \t {}'
    eval_temp = ' \t & {:.1f} & {:.1f} '
...
def print_ablation_study(mstr_bepoch_list, base_dir, base_mstr, other_mstrs, template, doc_type="overall",
                         metric_type='micro', data_type='test', span_type='pred_span'):
    ...
    base_eval = resume_eval_results(base_dir, data_type, span_type, base_mstr, model_str2best_epoch[base_mstr])[doc_type]
    model_str2eval_res = {
        model_str: resume_eval_results(base_dir, data_type, span_type, model_str, model_str2best_epoch[model_str])[doc_type]
