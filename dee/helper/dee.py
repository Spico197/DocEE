...
class DEEExample(object):
    def __init__(self, annguid, detail_align_dict, template, tokenizer, only_inference=False, inlcude_complementary_ents=False):
        self.guid = annguid
        # [sent_text, ...]
        ...
        if inlcude_complementary_ents:
            self.complementary_field2ents = regex_extractor.extract_doc(detail_align_dict['sentences'])
        else:
            ...
            if inlcude_complementary_ents:
                ...
                for raw_field, ents in self.complementary_field2ents.items():
                    ...
                    for ent, pos_span in ents:
                        ...
                        if ent not in detail_align_dict['ann_valid_mspans']:
                            comp_ents_sent_index[pos_span[0]].append([ent, raw_field, pos_span])
                            comp_ents_start_index[(pos_span[0], pos_span[1])].append([ent, raw_field, pos_span])
                            comp_ents_end_index[(pos_span[0], pos_span[2])].append([ent, raw_field, pos_span])
                ...
                for ent, field in mspan2fields.items():
                    for drange in mspan2dranges[ent]:
                        for s_ent in comp_ents_sent_index.get(drange[0], []):
                            ...
                            if drange[1] <= pos_span[1] < drange[2] or drange[1] < pos_span[2] <= drange[2]:
                                if [s_ent, pos_span] in self.complementary_field2ents[raw_field]:
                                    self.complementary_field2ents[raw_field].remove([s_ent, pos_span])
                ...
                for raw_field, ents in self.complementary_field2ents.items():
                    ...
                    for ent, pos_span in ents:
                        ...
                        elif list(pos_span) not in detail_align_dict['ann_mspan2dranges'][ent]:
            ...
            for mspan in self.ann_valid_mspans:
                for drange in self.ann_mspan2dranges[mspan]:
                    ...
                    if sent_text[char_s: char_e] != tokenizer.dee_tokenize(mspan):
                        raise ValueError('GUID: {} span range is not correct, span={}, range={}, sent={}'.format(
...
class DEEExampleLoader(object):
    def __init__(self, template, tokenizer, rearrange_sent_flag, max_sent_len,
    ...
    def drop_irr_ents(self, detail_align_info):
        ...
        for span in real_valid_spans:
            ...
            ann_mspan2guess_field[span] = detail_align_info['ann_mspan2guess_field'][span]
...
class DEEFeature(object):
    ...
    def build_key_event_sent_info(self):
        ...
        for sent_idx in range(self.valid_sent_num):  # masked sents will be truncated at the model part
            ...
            doc_sent_labels.append(sent_labels)  # tzhu: the shape is: #sent x #event_type
