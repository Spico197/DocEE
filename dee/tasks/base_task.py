...
class BasePytorchTask(object):
    ...
    def _load_data(self, load_example_func, convert_to_feature_func, convert_to_dataset_func,
        ...
        if load_inference:
            ...
            load_example_func = functools.partial(load_example_func, only_inference=True)
    ...
    def reset_bert_optimizer(self):
        ...
        scheduler = get_linear_schedule_with_warmup(
            optimizer,
            self.setting.warmup_proportion * num_train_steps,
            num_train_steps
        )
    ...
    def base_train(self, get_loss_func, kwargs_dict1={},
        ...
        for epoch_idx in trange(base_epoch_idx, int(self.setting.num_train_epochs), desc="Epoch", ncols=80, ascii=True):
            self.model.train()
            ...
            for step, batch in step_batch_iter:
                ...
                if self.setting.global_grad_clip:
                    ...
                    torch.nn.utils.clip_grad_norm_(self.model.biaffine.parameters(), 5.0)
                ...
                if (step + 1) % self.setting.gradient_accumulation_steps == 0:
                    ...
                    if self.is_master_node():
                        if isinstance(total_loss, dict):
                            self.summary_writer.add_scalars('Loss', total_loss, global_step=global_step)
                            pbar.set_postfix({"loss": total_loss['loss'].item()})
                        else:
                            self.summary_writer.add_scalar('Loss', total_loss.item(), global_step=global_step)
