...
from dee.helper import (
    ...
    match_arg,
    DEEExample, DEEExampleLoader,
    DEEFeatureConverter, convert_dee_features_to_dataset,
    DEEArgRelFeatureConverter, convert_dee_arg_rel_features_to_dataset,
...
from dee.models import (
    DCFEEModel,
    Doc2EDAGModel, LSTMMTL2EDAGModel,
)
...
class DEETaskSetting(TaskSetting):
    ...
    base_attr_default_pairs = [
        ...
        ('bert_model', '/home/tzhu/bert-pretrained-models/bert-base-chinese'),  # use which pretrained bert model
...
class DEETask(BasePytorchTask):
    ...
    def __init__(
        self, dee_setting,
        load_train=True, load_dev=True,
        load_test=True, load_inference=False,
        ...
        self.tokenizer = BertTokenizerForDocEE.from_pretrained(self.setting.bert_model, doc_lang=self.setting.doc_lang)
        ...
        self.event_type_fields_pairs = DEEExample.get_event_type_fields_pairs(self.event_template)
        ...
        if not self.setting.use_token_role:
            ...
            if self.setting.model_type != 'Doc2EDAG':
                logger.warning("Model is not Doc2EDAG! Make sure you know what you are doing here.")
        ...
        self.setting.ent_type2id = {
            event_type: idx for idx, event_type in
            enumerate([x[2:] for x in filter(lambda x: x.startswith("B-"), self.setting.tag_id2tag_name.values())])}
        ...
        elif self.setting.model_type in {"LSTMMTL", "LSTMMTL2EDAG"}:
            self.model = LSTMMTL2EDAGModel(
                self.setting, self.event_type_fields_pairs, ner_model=ner_model
            )
        ...
        for name, param in self.model.named_parameters():
            ...
            if param.requires_grad:
                logger.info("Trainable: {:20}\t{:20}\t{}".format(name, str(param.size()), param_num))
                all_trainable.append(param_num)
            else:
                logger.info("Untrainable: {:20}\t{:20}\t{}".format(name, str(param.size()), param_num))
        ...
        if self.setting.use_bert:
            ...
            if self.setting.optimizer == 'adamw':
                ...
                self.optimizer = optim.SGD(self.model.parameters(), lr=self.setting.learning_rate, momentum=0.9)
            else:
        ...
        # load data
        self._load_data(
            self.example_loader_func, self.feature_converter_func, convert_dataset_func,
    ...
    def get_event_idx2entity_idx2field_idx(self):
        ...
        for event_idx, (event_name, field_types, _, _) in enumerate(self.event_type_fields_pairs):
    ...
    def get_loss_on_batch(self, doc_batch_dict, features=None):
        ...
        try:
            ...
            logger.info('-' * 30)
            logger.info('Exception occurs when processing ' + ','.join([features[ex_idx].guid for ex_idx in doc_batch_dict['ex_idx']]))
    ...
    def get_event_decode_result_on_batch(self, doc_batch_dict, features=None, use_gold_span=False, heuristic_type=None):
        ...
        batch_eval_results = self.model(
            doc_batch_dict, features,
            use_gold_span=use_gold_span, train_flag=False,
    ...
    def resume_save_eval_at(self, epoch, resume_cpt_flag=False, save_cpt_flag=True):
        ...
        for task_idx, (data_type, gold_span_flag, heuristic_type) in enumerate(eval_tasks):
            ...
            _, measures = self.eval(features, dataset,
                                    use_gold_span=gold_span_flag, heuristic_type=heuristic_type,
            ...
            if self.is_master_node() and data_type == 'dev' and gold_span_flag is False:
                ...
                self.logging(f"Epoch: {epoch}, Current F1: {curr_f1 * 100:.3f}, Best F1: {self.best_f1 * 100:.3f}, is the best: {curr_f1 > self.best_f1}")
    ...
    def eval(self, features, dataset, use_gold_span=False, heuristic_type=None,
        ...
        if self.is_master_node():
            ...
            if dump_eval_json_name is not None:
                ...
                epoch = dump_eval_json_name.split('.')[-2]
            epoch = int(epoch)
            measure_list = ['classification', 'entity', 'combination', 'overall', 'instance']
            if self.summary_writer is not None:
                for measure_name in measure_list:
                    self.summary_writer.add_scalars(
                        f"{dataset_name}/{span_type}/{measure_name}", {
                            ...
                            "overall": total_eval_res["overall"][measure_name]['MicroF1'],
                        }, global_step=epoch
                    )
                ...
                for doc_type in ['o2o', 'o2m', 'm2m', 'overall']:
                    if "adj_mat" in total_eval_res[doc_type]:
                        adj_mat_measures.update({doc_type: total_eval_res[doc_type]["adj_mat"]['Accuracy']})
                    if "rawCombination" in total_eval_res[doc_type]:
                        raw_combination_measures.update({doc_type: total_eval_res[doc_type]["rawCombination"]['MicroF1']})
                    if "connection" in total_eval_res[doc_type]:
                        connection_measures.update({doc_type: total_eval_res[doc_type]["connection"]['MicroF1']})
                    if "trigger" in total_eval_res[doc_type]:
                        trigger_measures.update({doc_type: total_eval_res[doc_type]["trigger"]['MicroF1']})

                if adj_mat_measures:
                    self.summary_writer.add_scalars(
                        f"{dataset_name}/{span_type}/adj_mat", adj_mat_measures, global_step=epoch)
                if raw_combination_measures:
                    self.summary_writer.add_scalars(
                        f"{dataset_name}/{span_type}/rawCombination", raw_combination_measures, global_step=epoch)
                if connection_measures:
                    self.summary_writer.add_scalars(
                        f"{dataset_name}/{span_type}/connection", connection_measures, global_step=epoch)
                if trigger_measures:
                    self.summary_writer.add_scalars(
                        f"{dataset_name}/{span_type}/trigger", trigger_measures, global_step=epoch)

        return total_event_decode_results, total_eval_res

    def reevaluate_dee_prediction(self, max_epoch=100,
        ...
        pkl_match_name = os.path.join(eval_dir_path, f"{target_file_pre}.*{target_file_suffix}")
        ...
        for fn in pbar:
            ...
            if fn.startswith(target_file_pre) and fn.endswith(target_file_suffix) and len(fn_splits) == 6:
                ...
                total_eval_res = measure_dee_prediction(
                    self.event_template.event_type_fields_list, features, event_decode_results,
                    self.setting.event_relevant_combination,
                )
                ...
                for doc_type in ["o2o", "o2m", "m2m", "overall"]:
                    ...
                    if data_span_type not in doc_type2data_span_type2model_str2epoch_res_list[doc_type]:
                        doc_type2data_span_type2model_str2epoch_res_list[doc_type][data_span_type] = {}
                    model_str2epoch_res_list = doc_type2data_span_type2model_str2epoch_res_list[doc_type][data_span_type]
        ...
        for doc_type, data_span_type2model_str2epoch_res_list in doc_type2data_span_type2model_str2epoch_res_list.items():
    ...
    def ensemble_dee_prediction(self, curr_best_pkl_filepath, esmb_best_pkl_filepath):
        ...
        total_eval_res = measure_dee_prediction(
            self.event_template.event_type_fields_list, self.test_features, new_results,
            self.setting.event_relevant_combination,
        )
        print_data = []
        results = {
    ...
    def predict_one(self, string):
        ...
        examples = [self.example_loader_func.convert_dict_to_example(data[0], data[1], only_inference=True)]
        ...
        for drange, ment_type in zip(mention_drange_list, mention_type_list):
            mspan = self.tokenizer.convert_ids_to_tokens(doc_token_ids[drange[0]][drange[1]:drange[2]])
            if all(x.upper() != '[UNK]' for x in mspan):
                ...
                mspan, matched_drange = match_arg(example.sentences, doc_fea.doc_token_ids.numpy(), doc_token_ids[drange[0]][drange[1]:drange[2]], offset=int(self.feature_converter_func.include_cls))
        ...
        for event_idx, events in enumerate(result[2]):
            ...
            for ins in events:
                if all(x is None for x in ins):
                    continue
                tmp_ins = {
                    "event_type": self.event_template.event_type_fields_list[event_idx][0],
                    "arguments": None
                }
                arguments = []
                for field_idx, args in enumerate(ins):
                    ...
                    for arg in args:
                        ...
                        if all(x.upper() != '[UNK]' for x in arg_tmp):
                            ...
                            real_arg, _ = match_arg(example.sentences, doc_fea.doc_token_ids.numpy(), arg, offset=int(self.feature_converter_func.include_cls))
                            if real_arg is None:
                                self.logging(f"doc: {doc_id}, arg ({arg_tmp}) with UNK but original text not found", level=logging.WARNING)
                                real_arg = arg_tmp
                        arguments.append({"role": self.event_template.event_type_fields_list[event_idx][1][field_idx], "argument": real_arg})
    ...
    def debug_display(self, doc_type, span_type, epoch, midout_dir):
        ...
        from dee.utils import extract_combinations_from_event_objs, fill_diag, recover_ins, convert_role_fea_event_obj_to_standard
        ...
        for doc_fea, decode_result in zip(features, new_event_decode_results):
            if doc_type != "overall" and doc_fea.doc_type != {"o2o": 0, "o2m": 1, "m2m": 2}[doc_type]:
                continue
        ...
        for term in filtered_event_decode_results:
            ...
            if is_cg:
                ...
                for combinations_ in event_idx2combinations:
                    combinations_ = [tuple(sorted([doc_span_info.span_token_tup_list[arg] for arg in comb])) for comb in combinations_]
                ...
                doc_fea.event_arg_idxs_objs_list = remove_event_obj_roles(doc_fea.event_arg_idxs_objs_list, self.event_type_fields_pairs)
                tmp_gold_combinations = extract_combinations_from_event_objs(doc_fea.event_arg_idxs_objs_list)
                tmp_gold_combinations = set([tuple(sorted([doc_fea.span_token_ids_list[arg] for arg in comb])) for comb in tmp_gold_combinations])
            ...
            gold_record_mat = [
                ...
                ] if event_arg_idxs_objs is not None else None  # for events in each event type
        ...
        g_eval_res = measure_event_table_filling(
            ...
            pred_adj_mats=pred_adj_mats, gold_adj_mats=gold_adj_mats,
            pred_combinations=pred_combinations, gold_combinations=gold_combinations,
            dict_return=True
        )
        print_data = {
            "classification": {"p": g_eval_res["classification"]['MicroPrecision'], "r": g_eval_res["classification"]["MicroRecall"], "f1": g_eval_res["classification"]["MicroF1"]},
            "entity": {"p": g_eval_res["entity"]['MicroPrecision'], "r": g_eval_res["entity"]["MicroRecall"], "f1": g_eval_res["entity"]["MicroF1"]},
            "combination": {"p": g_eval_res["combination"]['MicroPrecision'], "r": g_eval_res["combination"]["MicroRecall"], "f1": g_eval_res["combination"]["MicroF1"]},
            "rawCombination": {"p": g_eval_res["rawCombination"]['MicroPrecision'], "r": g_eval_res["rawCombination"]["MicroRecall"], "f1": g_eval_res["rawCombination"]["MicroF1"]},
            "overall": {"p": g_eval_res["overall"]['MicroPrecision'], "r": g_eval_res["overall"]["MicroRecall"], "f1": g_eval_res["overall"]["MicroF1"]},
            "instance": {"p": g_eval_res["instance"]['MicroPrecision'], "r": g_eval_res["instance"]["MicroRecall"], "f1": g_eval_res["instance"]["MicroF1"]},
        ...
        for doc_fea, \
            pred_record_mat, gold_record_mat, \
            pred_event_type, gold_event_type, \
            pred_spans_token_tuple, gold_spans_token_tuple, \
            pred_adj_mat, gold_adj_mat, \
            pred_combination, gold_combination in zip(
                new_features,
                pred_record_mat_list, gold_record_mat_list,
                pred_event_types, gold_event_types,
                pred_spans_token_tuple_list, gold_spans_token_tuple_list,
                pred_adj_mats, gold_adj_mats,
                pred_combinations, gold_combinations):
            if pred_record_mat != gold_record_mat:
                continue
            texts = []
            for line in doc_fea.doc_token_ids:
                texts.append("".join(filter(lambda x: x != '[PAD]', tokenizer.convert_ids_to_tokens(line.tolist()))))
            mid_result = {
                "doc_type": ["o2o", "o2m", "m2m", "unk"][doc_fea.doc_type],
                "span_type": span_type,
                "guid": doc_fea.guid,
                "texts": texts,
                "pred_event_type": list(map(lambda x: x[0], filter(lambda x: x[1] == 1, zip(type_names, pred_event_type)))),
                "gold_event_type": list(map(lambda x: x[0], filter(lambda x: x[1] == 1, zip(type_names, gold_event_type)))),
                "pred_ents": ["".join(tokenizer.convert_ids_to_tokens(span)) for span in pred_spans_token_tuple],
                "gold_ents": ["".join(tokenizer.convert_ids_to_tokens(span)) for span in gold_spans_token_tuple],
                "pred_adj_mat": fill_diag(pred_adj_mat[0], -1),
                "gold_adj_mat": gold_adj_mat[0],
                "pred_comb": [["".join(tokenizer.convert_ids_to_tokens(arg)) for arg in comb] for comb in pred_combination],
                "gold_comb": [["".join(tokenizer.convert_ids_to_tokens(arg)) for arg in comb] for comb in gold_combination],
                "pred_ins": recover_ins(self.event_template.event_type_fields_list, tokenizer.convert_ids_to_tokens, pred_record_mat),
                "gold_ins": recover_ins(self.event_template.event_type_fields_list, tokenizer.convert_ids_to_tokens, gold_record_mat),
            }
            with open(os.path.join(output_dir, f"{doc_fea.guid}.json"), "wt", encoding="utf-8") as fout:
    ...
    def inference(self, dump_filepath=None, resume_epoch=1):
        ...
        self.logging('=' * 20 + 'Start Inference, Will Dump to: ' + dump_filepath + '=' * 20)
        ...
        for step, batch in enumerate(tqdm(eval_dataloader, desc=iter_desc, ncols=80, ascii=True)):
            batch = self.set_batch_to_device(batch)
            ...
            with torch.no_grad():
                ...
                batch_info = self.get_event_decode_result_on_batch(
                    batch, features=self.inference_features, use_gold_span=False, heuristic_type=None)
        ...
        assert len(self.inference_examples) == len(self.inference_features) == len(total_info)
        ...
        if dump_filepath is not None:
            with open(dump_filepath, "wt", encoding="utf-8") as fout:
                for example, doc_fea, result in zip(example_list, feature_list, total_info):
                    ...
                    doc_res = {
                        ...
                        "comments": {"pred_types": event_types, "mspans": mspans, "sentences": example.sentences},
                    }
