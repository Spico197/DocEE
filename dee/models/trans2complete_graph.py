...
class Trans2CompleteGraphModel(nn.Module):
    ...
    def __init__(self, config, event_type_fields_pairs, ner_model=None):
        ...
        self.event_tables = nn.ModuleList([
            EventTableForArgRel(event_type, field_types, config.hidden_size, min_field_num)
        ...
        if self.config.use_span_lstm_projection:
            ...
            self.start_mlp = nn.Linear(self.config.hidden_size, self.config.biaffine_hidden_size)
            self.end_mlp = nn.Linear(self.config.hidden_size, self.config.biaffine_hidden_size)
    ...
    def forward(self, doc_batch_dict, doc_features,
        ...
        doc_arg_rel_info_list = get_doc_arg_rel_info_list(doc_token_types_list, doc_fea_list, self.event_type_fields_pairs, use_gold_span=use_gold_span)
    ...
    def get_doc_span_mention_emb(self, doc_token_emb, doc_arg_rel_info):
        ...
        if len(doc_arg_rel_info.mention_drange_list) == 0:
            ...
            mention_sent_id_list = [drange[0] for drange in doc_arg_rel_info.mention_drange_list]
    ...
    def get_arg_combination_loss(self, biaffine_out, doc_arg_rel_info, event_idx=None):
        if self.config.event_relevant_combination:
            rel_adj_mat = torch.tensor(doc_arg_rel_info.event_arg_rel_mats[event_idx].reveal_adj_mat(),
                                       dtype=torch.long, device=biaffine_out.device,
                                       requires_grad=False)
        else:
            rel_adj_mat = torch.tensor(doc_arg_rel_info.whole_arg_rel_mat.reveal_adj_mat(),
                                       dtype=torch.long, device=biaffine_out.device,
                                       requires_grad=False)
        ...
        if self.config.add_adj_mat_weight_bias:
            combination_loss = F.cross_entropy(
                result_tensor, rel_adj_mat,
                ignore_index=-1, reduction='sum',
                weight=torch.tensor([1.0, 0.5], device=biaffine_out.device))
        else:
            combination_loss = F.cross_entropy(result_tensor, rel_adj_mat, ignore_index=-1, reduction='sum')
        return combination_loss

    def get_arg_role_loss(self, arg_role_logits, role_types):
        role_types_tensor = torch.tensor(role_types, dtype=torch.long,
    ...
    def get_loss_on_doc(self, doc_token_emb, doc_sent_emb,
        ...
        if self.config.use_span_att:
            batch_span_context = self.span_att(batch_span_context, batch_span_context, batch_span_context)
        ...
        if not self.config.event_relevant_combination:
            ...
            assert biaffine_out.shape[-1] == doc_arg_rel_info.whole_arg_rel_mat.len_spans
            comb_loss = self.get_arg_combination_loss(biaffine_out, doc_arg_rel_info, event_idx=None)
                ...
                if events is not None:
                    for event_instance in events:
                        ...
                        for span_idx, role_type in event_instance:
                            ...
                            span_rep_list_for_event_instance.append(span_context_list[span_idx])
                        span_rep_for_event_instance = torch.cat(span_rep_list_for_event_instance, dim=0)
            ...
            for event_idx, event_label in enumerate(doc_fea.event_type_labels):
                ...
                biaffine_out = self.get_adj_mat_logits(batch_span_context)
                assert biaffine_out.shape[-1] == doc_arg_rel_info.event_arg_rel_mats[event_idx].len_spans
                comb_loss = self.get_arg_combination_loss(biaffine_out, doc_arg_rel_info, event_idx=event_idx)
                arg_combination_loss.append(comb_loss)
    ...
    def get_mix_loss(self, doc_sent_loss_list, doc_event_loss_list, doc_arg_rel_info_list):
        ...
        doc_ner_loss_list = []
        for doc_sent_loss, doc_span_info in zip(doc_sent_loss_list, doc_arg_rel_info_list):
    ...
    def get_gold_results_on_doc(self, doc_token_emb, doc_sent_emb, doc_fea, doc_arg_rel_info):
        """
        ...
        span_mention_range_list, mention_drange_list, mention_type_list = \
            get_span_mention_info(doc_fea.span_dranges_list, doc_fea.doc_token_labels.detach().cpu().tolist())

        doc_arg_rel_info = DocArgRelInfo(
            doc_fea.span_token_ids_list,
            [x in doc_fea.exist_span_token_tup_set for x in doc_fea.span_token_ids_list],
        ...
        if not self.config.event_relevant_combination:
            pred_adj_mat = torch.tensor(doc_fea.whole_arg_rel_mat.reveal_adj_mat(), device=batch_span_context.device, requires_grad=False)
            pred_adj_mat = self.pred_adj_mat_reorgnise(pred_adj_mat)
            pred_adj_mat = pred_adj_mat.detach().cpu().tolist()
            final_pred_adj_mat.append(pred_adj_mat)
            raw_combinations = bron_kerbosch_pivoting_decode(pred_adj_mat, 3)
            ...
            for event_idx, event_pred in enumerate(event_pred_list):
                ...
                pred_adj_mat = torch.tensor(doc_fea.whole_arg_rel_mat.reveal_adj_mat(), device=batch_span_context.device)
                pred_adj_mat = self.pred_adj_mat_reorgnise(pred_adj_mat)
    ...
    def get_event_field_span_candidates(self, span_token_tup_list, span_mention_range_list,
        ...
        for event_idx, (event_type, field_types, _) in enumerate(self.event_type_fields_pairs):
