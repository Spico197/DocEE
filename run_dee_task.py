...
from dee.helper import aggregate_task_eval_info, print_total_eval_info, print_single_vs_multi_performance
...
def parse_args(in_args=None):
    ...
    arg_parser.add_argument('--template_filepath', type=str, default='result_template.html',
    ...
    arg_parser.add_argument('--load_dev', type=strtobool, default=True,
                            help='Whether to load dev')
    arg_parser.add_argument('--load_test', type=strtobool, default=True,
                            help='Whether to load test')
    arg_parser.add_argument('--load_inference', type=strtobool, default=False,
                            help='Whether to load inference data')
    arg_parser.add_argument('--inference_epoch', type=int, default=-1,
                            help='which epoch to load for inference')
    arg_parser.add_argument('--run_inference', type=strtobool, default=False,
                            help='Whether to run inference process')
    arg_parser.add_argument('--inference_dump_filepath', type=str, default="./inference.json",
                            help='dumped inference results filepath')
    arg_parser.add_argument('--ensemble', type=strtobool, default=False,
                            help='ensembling')
    arg_parser.add_argument('--ensemble_curr_best_pkl_filepath', type=str,
                            help='ensemble_curr_best_pkl_filepath')
    arg_parser.add_argument('--ensemble_esmb_best_pkl_filepath', type=str,
                            help='ensemble_esmb_best_pkl_filepath')
    arg_parser.add_argument('--speed_test', type=strtobool, default=False,
                            help='speed test mode')
    arg_parser.add_argument('--speed_test_epochs', type=int, default=10,
                            help='speed test epoch')
    arg_parser.add_argument('--debug_display', type=strtobool, default=False,
                            help='debug display')
    arg_parser.add_argument('--debug_display_midout_dir', type=str,
                            help='debug display mid output directory')
    arg_parser.add_argument('--debug_display_epoch', type=int, default=1,
                            help='debug display epoch')
    arg_parser.add_argument('--debug_display_doc_type', type=str, default="o2o",
                            choices=["o2m", "o2o", "m2m", "overall"],
                            help='debug display doc type')
    arg_parser.add_argument('--debug_display_span_type', type=str, default="pred_span",
                            choices=["pred_span", "gold_span"],
                            help='debug display span type')
    arg_parser.add_argument('--eval_model_names', type=str,
                            default='DCFEE-O,DCFEE-M,GreedyDec,Doc2EDAG,LSTMMTL,LSTMMTL2CompleteGraph,' + ','.join(list_models()),
    ...
    arg_parser.add_argument('--parallel_decorate', action='store_true', default=False,
...
if __name__ == '__main__':
    ...
    ############## os.environ["CUDA_VISIBLE_DEVICES"] = in_argv.cuda_visible_devices
    ...
    dee_task = DEETask(dee_setting, load_train=not in_argv.skip_train,
                       load_dev=in_argv.load_dev, load_test=in_argv.load_test,
    ...
    if in_argv.speed_test:
        ...
        best_epoch = print_best_test_via_dev(
            in_argv.task_name,
            dee_setting.model_type,
            dee_setting.num_train_epochs
        )
        ...
        for i in range(in_argv.speed_test_epochs):
            ...
            for step, batch in enumerate(pbar):
                ...
                with torch.no_grad():
                    ...
                    batch_info = dee_task.get_event_decode_result_on_batch(batch, **func_kwargs)
                    used_time = time.time() - start_time
                    total_time.append(used_time)

            logger.info(pbar)

        logger.info(f"Task: {in_argv.task_name}, Model: {dee_setting.model_type}, eval batchsize: {in_argv.eval_batch_size}")
        logger.info(f"Speed test: #docs: {num_docs}, #batches: {num_batch}, #eval_epochs: {in_argv.speed_test_epochs}")
        logger.info(f"Total used time: {sum(total_time):.3f}, avg time per batch: {statistics.mean(total_time):.3f}")
        logger.info(f"Inference speed (docs): {in_argv.speed_test_epochs * num_docs / sum(total_time):.3f} docs/s")
    ...
    if in_argv.run_inference:
        ...
        dee_task.inference(resume_epoch=int(best_epoch), dump_filepath=in_argv.inference_dump_filepath)
        sys.exit(0)
    ...
    if dee_task.is_master_node():
        ...
        sm_results = print_single_vs_multi_performance(
            mstr_bepoch_list, in_argv.output_dir, dee_task.test_features, dee_task.event_template,
        ...
        for model_type in model_types:
            best_epoch = print_best_test_via_dev(
                in_argv.task_name, model_type, in_argv.num_train_epochs,
                span_type=span_type, data_type=doc_type,
                measure_key="MicroF1" if metric_type == "micro" else "MacroF1")
            pred_result = print_detailed_specified_epoch(in_argv.task_name, model_type, best_epoch,
                                                         span_type="pred_span")
            pred_results.append(pred_result)
            gold_result = print_detailed_specified_epoch(in_argv.task_name, model_type, best_epoch,
                                                         span_type="gold_span")
        ...
        html_results = render_results(
            in_argv.template_filepath,
            data=html_data
        )
        with open(os.path.join(task_dir, "results-{}.html".format(in_argv.task_name)), "wt", encoding="utf-8") as fout:
            fout.write(html_results)
        if not os.path.exists("./Results/html"):
            os.makedirs("./Results/html")
        with open(os.path.join("./Results/html", "results-{}.html".format(in_argv.task_name)), "wt", encoding="utf-8") as fout:
            fout.write(html_results)
        if not os.path.exists("./Results/data"):
            os.makedirs("./Results/data")
        with open(os.path.join("./Results/data", "data-{}.json".format(in_argv.task_name)), "wt", encoding="utf-8") as fout:
